...
class BinaryTree <T: Comparable>{
    ...
    open func afterRemove(node:AVLNode<T>?){}
...
extension BinaryTree{
     ...
     private func preOrderTraversal(node : AVLNode<T>?) -> Void {
         if node == nil {
             return
         }
     ...
     private func inOrderTraversal(node : AVLNode<T>?) -> Void {
        if node == nil {
            return
        }
      ...
      private func postOrderTraversal(node : AVLNode<T>?) -> Void {
         if node == nil {
             return
         }
    ...
    func levelOrder(_ root: AVLNode<T>?) -> [[T]]{
         guard root != nil else {
            return []
        }
        var lastRes = [[T]]()
...
extension BinaryTree{
    ...
    private func height1(node:AVLNode<T>?) -> Int {
        if node == nil {
            return 0
        }
    ...
    private func height2(node:AVLNode<T>?) -> Int {
        if node == nil {
            return 0
        }
        var levelSize = 1 //存储每次的元素数量 数组的count
        var height = 0
        var queue = [AVLNode<T>]() //队列数组
        queue.append(root!)
        while !queue.isEmpty {
            let node = queue.removeFirst()
...
extension BinaryTree{
    ...
    func widthOfBinaryTree(node:AVLNode<T>?) -> Int {
        if root == nil {
            return 0
        }
        var queue : [AVLNode<T>] = [root!] //队列数组
        var list : [Int] = [1]
...
extension BinaryTree{
    //前驱
    func predecessor(node : AVLNode<T>?) -> AVLNode<T>?{
        var node = node
    ...
    //后继
    func successor(node : AVLNode<T>?) -> AVLNode<T>?{
        var node = node
