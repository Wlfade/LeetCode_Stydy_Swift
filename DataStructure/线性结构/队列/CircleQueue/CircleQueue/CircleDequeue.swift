...
class CircleDequeue<T>: NSObject {
    ...
    public func deQueueRear() -> T?{
        ...
        let rearElement = elements[(front + size - 1) % elements.count]
        
        elements[(front + size - 1) % elements.count] = defaultValue
        size -= 1
        return rearElement
    }
    /// 从对头出队
    ...
    private func index(_ index : Int) -> Int{
        ...
        return theIndex - ((theIndex >= elements.count) ? elements.count : 0)
    }
}
//MARK:- 扩容 缩容
extension CircleDequeue{
    /// 扩容操作
